...
- **🔧 广东省爬虫特化策略**：
  - **⚠️ 重要提醒**：当前版本的广东省爬虫不建议使用，存在数据源和稳定性问题。
  - **推荐方案**：建议使用我的另一个开源项目 [gd-law-crawler](https://github.com/ViVi141/gd-law-crawler)与[MNR-Law-Crawler](https://github.com/ViVi141/MNR-Law-Crawler)该项目的数据采集源从第三方改为官方源，爬取效率和数据准确性更高。
  - 当前版本采用**广东省+中山市特化**策略，只爬取省级和中山市相关机构的政策数据。
  - 使用发布机关精确筛选，自动排除其他20个地级市的政策。
  - 支持省级和中山市相关机构：广东省人大、广东省人民政府、广东省其他机构、中山市人民政府、中山市其他机构。
  - ⚠️ **开发中**：全省全市通用爬取功能仍在开发中，敬请期待后续版本。
...
## 🆕 3.1.2版本更新内容（2025.11.13）

### 🔧 运行稳定性
- **入口页预热与 Cookie 更新**：所有广东爬虫会话在初始化与轮换后自动访问对应 `adv` 页面，确保最新 Cookie 和隐藏字段同步，减少被动重定向。
- **限流退避梯度**：检测到“全文访问限制”时按次数递增等待，并强制刷新代理+会话，避免短时间持续触发封禁。

### 🛡️ 防护策略
- **浏览器指纹增强**：默认补齐 `Accept-Language`、`Accept-Encoding`、`Sec-Fetch-*`、`sec-ch-ua` 等头部，保持与真实浏览器一致的指纹组合。
- **节奏更保守**：广东站默认延迟调整为 3~7 秒，速率上限 20 RPM，并在所有请求前执行统一的随机延时。

### 🎯 广东省爬虫特化策略
- **广东省+中山市特化**：广东省爬虫现已改为特化策略，只爬取省级和中山市相关机构的政策数据。
- **发布机关精确筛选**：使用发布机关lvalue值进行精确筛选，支持省级机构（广东省人大、广东省人民政府、广东省其他机构）和中山市机构（中山市人民政府、中山市其他机构）。
- **开发中**：全省全市通用爬取功能仍在开发中，敬请期待后续版本。

### 📚 文档与版本
- 更新 `README.md`、`CHANGELOG.md`、`版本管理说明.md` 记录 3.1.2 版本要点。
- `__version__`、`APP_CONFIG['app_version']`、打包脚本同步升版。
