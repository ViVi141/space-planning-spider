...
### 🔧 改进
- **入口页预热**：广东爬虫会话初始化与轮换后自动访问对应 `adv` 页面，同步最新 Cookie / 隐藏字段，减轻访问重定向与403风险。
- **节奏与限流控制**：统一将广东站默认延迟调至 3~7 秒、速率上限 20 RPM；检测到全文访问限制时按次数递增退避，并强制刷新代理与会话。
- **指纹伪装增强**：默认补齐 `Accept-Language`、`Accept-Encoding`、`Sec-Fetch-*`、`sec-ch-ua` 等头部字段，进一步贴近真实浏览器请求。
- **详情页拦截反馈**：保存限制快照、记录 `access_blocked` 标记，并在多次受限后自动触发会话重建。

### 📝 文档
- 更新 `README.md`、`CHANGELOG.md`、`版本管理说明.md` 说明 3.1.2 的防封策略调整。
- 同步版本号至 3.1.2（`__version__`、`APP_CONFIG['app_version']`、打包脚本输出）。
...
### ✨ 新增
- **统一防反爬配置中心**：所有国家/广东/自然资源部（含多线程）爬虫接入 `AntiCrawlerManager`，与 GUI「爬虫设置」保持实时同步。

### 🔧 改进
- **会话与代理统一管理**：轮换会话、Cookie 管理、频率限制、随机 UA/指纹、行为模拟、共享代理刷新等能力全部集中处理，消除各爬虫重复实现的分散逻辑。
- **请求节奏一致化**：移除硬编码延时与局部速率控制，速度模式、随机延迟、指数退避重试等均按配置执行，确保单线程/多线程行为一致。
- **多线程稳定性**：国家、自然资源部多线程版本改用统一请求通道，自动在并发环境中应用代理切换与限速策略。
- **成功阈值代理轮换**：新增 `rotate_after_success_count` 配置，允许在成功获取指定数量政策后强制刷新会话与代理，降低封禁风险。

### 📝 文档
- 更新 `README.md`、`CHANGELOG.md`、`版本管理说明.md`、`docs/guangdong_spider_data_flow.md` 说明集中化防反爬策略及配置入口。
